...
public class PipetteLoading extends HighLevelMotion {
	...
	boolean headMoved4 = false;
	
	public PipetteLoading() {
		
	} // close PipetteLoading constructor method
	...
	ActionListener stateTask = new ActionListener() {
		...
		public void actionPerformed(ActionEvent e) {
			...
			// STAGE 5 HEAD PIPETTE CAPTURE MOTION -----------------------------------------------------------
			if  (magnetRotated && magnetMoved && headMoved1 && robotMoved1 && !headMoved2 &&
					!(Machine.getDrawPanel().getController().getAutoMotion().getGraphicMotionTimer().isRunning())) {
				headMove(base, base.getCurrentStep(), base.getCurrentStep()+100);
				headMoved2 = true;
			} // close "if"

			// STAGE 6 HEAD RETURN MOTION -----------------------------------------------------------
			if  (magnetRotated && magnetMoved && headMoved1 && robotMoved1 && headMoved2 &&
				!headMoved3 &&
				!(Machine.getDrawPanel().getController().getAutoMotion().getGraphicMotionTimer().isRunning())) {
				headMove(base, 1);
			...
			// STAGE 7 ROBOT MOTION -----------------------------------------------------------
			if  (magnetRotated && magnetMoved && headMoved1 && robotMoved1 && headMoved2 &&
					headMoved3 && !robotMoved2 &&
					!(Machine.getDrawPanel().getController().getAutoMotion().getGraphicMotionTimer().isRunning())) {
				partMove(robot, 500, HIGH);
				robotMoved2 = true;
			} // close "if"

			// STAGE 8 HEAD Z TOP MOTION -----------------------------------------------------------
			if  (magnetRotated && magnetMoved && headMoved1 && robotMoved1 && headMoved2 &&
					headMoved3 && robotMoved2 && !headMoved4 &&
					!(Machine.getDrawPanel().getController().getAutoMotion().getGraphicMotionTimer().isRunning())) {
				headMove(base, 0);
				headMoved4 = true;
			} // close "if"
