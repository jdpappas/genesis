...
public class GraphPanel extends JComponent {
    ...
    public static void main(String[] args) throws Exception {
        EventQueue.invokeLater(new Runnable() {
            ...
            public void run() {
                ...
                // gp.control is the ControlPanel instance of the GraphPanel
                f.add(new JScrollPane(gp), BorderLayout.CENTER);
                f.getRootPane().setDefaultButton(gp.control.defaultButton);
                // defaultButton: The button that gets activated when the pane has the focus and 
    ...
    public void paintComponent(Graphics g) {
        g.setColor(new Color(0x00f0f0f0)); // the background color
        g.fillRect(0, 0, getWidth(), getHeight());
        
        for (Edge e : edges) {
            e.draw(g); // draws all the edges in the arraylist
        }
        for (Node n : nodes) {
            n.draw(g); // draws all the nodes in the arraylist
        }
    ...
    private class MouseHandler extends MouseAdapter {
        ...
        public void mouseReleased(MouseEvent e) {
            selecting = false;
            mouseRect.setBounds(0, 0, 0, 0); // actually deletes the selection rectangle when the mouse button is released 
            if (e.isPopupTrigger()) {
                showPopup(e);
            }
    ...
    private class ControlPanel extends JToolBar {
    	// Actually here we need mainly the methods of JToolbar, but we also add a popupmenu.
    ...
    } // close ColorIcon class
} // close GraphPanel class